...
### ADD ASPECT TO THIS LIST TO REMOVE ######
allmets <- alldata[,!names(alldata) %in% c("PlotID", "Dataset", "FireYear", "Start_Day", "End_Day", "Unit", "ID", "precip0", "precip1", "vpd0", "aet0", "aet1", "cwd0", "cwd1", "stdDevAETValue", "stdDevCWDValue", "stdDevPrecipValue", "stdDevVPDValue", "aspect")] #,"devi", "rbr", "dnbr"

names(allmets)
allmets <- allmets[!is.na(allmets$pcnt_ba_mo),]
corr <- round(cor(allmets[,2:29]), 1)
p.mat <- cor_pmat(allmets[,2:29])
str(allmets)
ggcorrplot(corr) 
...
alldat <- allmets[, -which(names(allmets) == "YrFireName")]
vsurf_res <- VSURF(pcnt_ba_mo ~ ., data = alldat, ntree = 500)  
selected_vars <- vsurf_res$varselect.pred
alldat[1, selected_vars]
selected_vars <- append(selected_vars, 1)
rf_vsurf_all <- randomForest(pcnt_ba_mo ~ ., data = alldat[, selected_vars], proximity = TRUE)

column_names <- names(alldat)
...
ggsave("VP/severity_tmp/plots/BAmortality.png", width = 6.4, height = 6.5, units = "in")
