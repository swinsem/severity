...
ggsave("VP/severity_tmp/plots/BA_dSWIR_precip.png", width = 3, height = 4, units = "in")
...
ggplot(ard, aes(x = dswir2swir1, y = pcnt_ba_mo, col = northness > 0)) +
  ...
  theme_minimal() +
  theme(legend.position="bottom", legend.direction="vertical")
ggsave("VP/severity_tmp/plots/BA_dSWIR_northness.png", width = 3, height = 4, units = "in")
...
combined_plot <- combined_plot +
  cowplot::draw_plot_label(
    label = c("a", "b"),
    x = c(0, 0.5),       # Adjust x positions as needed
    y = c(0, 0),         # y=0 places the labels at the bottom
...
# Save Combined Plot
ggsave("VP/severity_tmp/plots/BA_dSWIR_combined.png", 
